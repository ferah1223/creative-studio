...
export default function ShaderPlayground() {
  ...
  const selectPreset = (i: number) => {
    setPreset(i)
    setFragCode(presets[i].frag)
  }

  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display font-bold text-3xl tracking-tight" style={{ color: 'var(--color-cyan)' }}>
          Shader Playground
        </h1>
        <p className="text-ink-secondary mt-1">Live GLSL editor with real-time preview.</p>
      </div>
        ...
        {presets.map((p, i) => (
          <button key={p.name} onClick={() => selectPreset(i)}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium cursor-pointer transition-all ${
              preset === i ? 'bg-cyan text-white' : 'bg-surface-1 border border-border-subtle text-ink-secondary hover:border-cyan'
            }`}>
            {p.name}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-border-subtle bg-surface-1 overflow-hidden">
          <div className="p-3 border-b border-border-subtle flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={() => setRunning(r => !r)}
                className="w-7 h-7 rounded-md flex items-center justify-center bg-surface-2 hover:bg-surface-3 transition-colors cursor-pointer">
          ...
          <textarea
            ...
            spellCheck={false}
            className="w-full h-80 p-4 font-mono text-xs leading-relaxed bg-transparent resize-none outline-none"
            style={{ tabSize: 2 }}
          />
          {error && (
            <div className="px-4 py-2 bg-coral/10 text-coral font-mono text-xs border-t border-coral/20">
              {error}
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-border-subtle bg-surface-1 overflow-hidden">
          <div className="p-3 border-b border-border-subtle">
            <span className="font-mono text-xs text-ink-muted">Output</span>
          </div>
